...
interface Mentoria {
  ...
  agressorId: string;  // ID do mentorado (agressor)
  hour: string;  // Hora da mentoria
  date: string;  // Data da mentoria
}
  ...
  const fetchMentorias = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Modifica os dados para incluir apenas as informações necessárias
        const mentoriasComStatusNull = data.map((mentoria: Mentoria) => ({
          mentorId: mentoria.mentorId,
          agressorId: mentoria.agressorId,
          hour: mentoria.hour,
          date: mentoria.date,
        }));
  ...
  return (
    <div className="instituicao-container">
      ...
      <form onSubmit={handleCadastrarMentor} className="cadastro-container">
        ...
        <button type="submit" className='BLogin'>Cadastrar Mentor</button>
      </form>
        ...
        {mentorias.length > 0 ? (
          mentorias.map((mentoria, index) => (
            <div key={index} className="mentoria-item">
              <p><strong>ID do Mentor:</strong> {mentoria.mentorId}</p>
              <p><strong>ID do Mentorado:</strong> {mentoria.agressorId}</p>
              <p><strong>Data e Hora:</strong> {mentoria.date} às {mentoria.hour}</p>
            </div>
          ))
        ) : (
